 ...
 * - Enhanced patient-centric dashboard with charts, alerts, quick actions, records
 * - Dummy data only
 *
 * Requires:
 ...
 *
 * Integration:
 *  - Add route: <Route path="/patient" element={<PatientDashboard />} />
 */
...
export default function PatientDashboard() {
  ...
  // Health trend (recovery) dummy data — lower is better for symptom severity; higher wellbeing score is better
  const healthTrend = [
    { date: "Aug 28", wellbeing: 48, severity: 6 },
    { date: "Aug 31", wellbeing: 54, severity: 5 },
    { date: "Sep 03", wellbeing: 60, severity: 4 },
    { date: "Sep 06", wellbeing: 67, severity: 3 },
    { date: "Sep 09", wellbeing: 75, severity: 2 },
    { date: "Sep 12", wellbeing: 82, severity: 1 },
  ];

  // Med adherence + meds
  const [meds] = useState([
    { id: 1, name: "Amlodipine 5mg", dose: "1 × daily", adherence: 92 },
    { id: 2, name: "Atorvastatin 10mg", dose: "1 × nightly", adherence: 88 },
  ]);

  // Notices / local epidemic alerts - research-y style
  const [notices] = useState([
    {
      id: 1,
      title: "Dengue Alert — High Risk",
      level: "High",
      text: "Increased dengue vector activity reported in Ludhiana district. Use mosquito nets, eliminate standing water and visit clinic if fever > 38°C for 2 days.",
      date: "Sep 12, 2025",
    },
    {
      id: 2,
      ...
      level: "Medium",
      text: "Vaccination clinics available in select CHCs. Elderly and young children advised to get flu shot.",
      date: "Sep 05, 2025",
    },
  ]);

  // Appointments + recent consults
  const [appointments] = useState([
    { id: 1, doctor: "Dr. Sharma (Cardiology)", when: "Sep 28, 2025 — 11:00 AM", status: "Upcoming" },
  ]);
  const [consults] = useState([
    { id: 1, name: "Dr. Mehta", date: "Aug 30, 2025", note: "Follow-up: BP controlled. Continue meds." },
    { id: 2, name: "Dr. Verma", date: "Jun 10, 2025", note: "Routine check, recommended blood tests." },
  ]);

  // files
  const fileRef = useRef(null);
  const [files, setFiles] = useState([
    { id: 1, name: "CBC_Sep2025.pdf", uploadedAt: "2025-09-06" },
  ]);
  function handleFiles(e) {
    const list = Array.from(e.target.files || []);
    const newFiles = list.map((f, idx) => ({
      id: Date.now() + idx,
      name: f.name,
  ...
  // Quick print/download health card (same simple approach)
  function downloadHealthCard() {
    const html = `
      <html><head><title>Health Card</title></head><body>
      <div style="font-family: Arial, Helvetica, sans-serif; padding:20px;">
        <h2 style="color:#0f766e">SehatSathi — Health Card</h2>
        <p><strong>Name:</strong> ${profile.name}</p>
        <p><strong>ID:</strong> ${profile.id}</p>
        <p><strong>Phone:</strong> ${profile.phone}</p>
        <hr/>
        <p>Prescriptions and recent consults included.</p>
      </div></body></html>
    `;
  ...
  // small helpers
  const wellbeingAvg = Math.round((healthTrend[healthTrend.length - 1].wellbeing));
  const severityAvg = Math.round(healthTrend.reduce((s, r) => s + r.severity, 0) / healthTrend.length);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-lg bg-teal-50 flex items-center justify-center text-teal-700 text-3xl">
              <FaUserCircle />
            </div>
            <div>
              <div className="text-sm text-gray-500">Welcome back,</div>
              <div className="text-lg font-bold text-gray-800">{profile.name}</div>
            </div>
            <div className="ml-6">
              ...
              <div className="w-40 h-2 bg-gray-100 rounded-full mt-1 overflow-hidden">
                <div className="h-full bg-teal-600" style={{ width: `${profile.completed}%` }}></div>
              </div>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/symptom-checker")} className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 flex items-center gap-2">
              <FaRobot /> AI Checker
            </button>
            <button onClick={() => downloadHealthCard()} className="bg-white border px-4 py-2 rounded-lg flex items-center gap-2">
              <FaDownload /> Health Card
            </button>
            <button onClick={() => window.location.href = "tel:+919876543210"} className="bg-red-500 text-white px-3 py-2 rounded-lg flex items-center gap-2">
              <FaPhoneAlt /> SOS
            </button>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        {/* Top panels: Charts + Notices */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Health Trend Card */}
          <div className="bg-white rounded-2xl shadow p-4 col-span-2">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2"><FaChartLine /> Health Trend</h3>
                <p className="text-sm text-gray-500">Wellbeing score (0-100) vs symptom severity</p>
              </div>
              <div className="text-right">
                <div className="text-xs text-gray-500">Latest wellbeing</div>
                <div className="text-2xl font-bold text-teal-700">{wellbeingAvg}</div>
                <div className="text-xs text-gray-500 mt-1">Current severity: {severityAvg}/10</div>
              </div>
            </div>
            ...
            <div className="mt-4 h-56">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={healthTrend}>
                  ...
                  <Tooltip />
                  <Line yAxisId="left" type="monotone" dataKey="wellbeing" stroke="#0f766e" strokeWidth={3} dot={{ r: 3 }} />
                  <Line yAxisId="right" type="monotone" dataKey="severity" stroke="#ef4444" strokeWidth={2} dot={{ r: 2 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>

            <div className="mt-3 flex items-center gap-3 text-sm text-gray-600">
              <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-teal-600 inline-block" /> Wellbeing</div>
              <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-red-500 inline-block" /> Severity</div>
            </div>
          </div>
          ...
          <aside className="bg-white rounded-2xl shadow p-4">
            <div className="flex items-center justify-between">
              <h4 className="font-semibold text-gray-800 flex items-center gap-2"><FaBell /> Important Notices</h4>
              <button className="text-sm text-teal-600 hover:underline" onClick={() => navigate("/alerts")}>All</button>
            </div>
              ...
              {notices.map((n) => (
                <div key={n.id} className="border rounded-lg p-3">
                  <div className="flex items-start justify-between">
                    ...
                    <div>
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${n.level === "High" ? "bg-red-100 text-red-700" : n.level === "Medium" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>
                        {n.level}
                      </span>
                    </div>
                  </div>
        ...
        {/* Mid area: Appointments / Records */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Upcoming + Consults */}
          <div className="col-span-2 space-y-4">
            <div className="bg-white rounded-2xl shadow p-4">
              <div className="flex items-center justify-between">
                <h4 className="font-semibold text-gray-800"><FaCalendarAlt /> Upcoming Appointments</h4>
                <button className="text-sm text-teal-600 hover:underline" onClick={() => navigate("/appointments")}>Manage</button>
              </div>

              <div className="mt-3 space-y-3">
                {appointments.length === 0 ? (
                  <p className="text-sm text-gray-500">No upcoming appointments. Book one now.</p>
                ) : appointments.map(a => (
                  <div key={a.id} className="flex items-center justify-between border rounded-lg p-3">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className={`px-2 py-1 rounded-full text-xs ${a.status === "Upcoming" ? "bg-teal-50 text-teal-700" : "bg-gray-100 text-gray-600"}`}>{a.status}</div>
                      <div className="flex gap-2">
                        <button onClick={() => alert("Join video (demo)")} className="bg-white border px-3 py-1 rounded-md text-sm">Join</button>
                        <button onClick={() => navigate("/appointments")} className="bg-teal-600 text-white px-3 py-1 rounded-md text-sm">Details</button>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-2xl shadow p-4">
              <div className="flex items-center justify-between">
                <h4 className="font-semibold text-gray-800"><FaNotesMedical /> Recent Consultations</h4>
                <button className="text-sm text-teal-600 hover:underline" onClick={() => navigate("/consults")}>All</button>
              </div>
              ...
              <div className="mt-3 divide-y">
                {consults.map(c => (
                  <div key={c.id} className="py-3 flex items-start justify-between">
                    <div>
                      <div className="font-medium text-gray-800">{c.name} <span className="text-xs text-gray-500">• {c.date}</span></div>
                      <div className="text-sm text-gray-600 mt-1">{c.note}</div>
                    </div>
                    <div className="text-right">
                      <button className="text-sm bg-white border px-3 py-1 rounded-md">Download Notes</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Right column: Prescriptions + Upload */}
          <aside className="space-y-4">
            <div className="bg-white rounded-2xl shadow p-4">
              <h4 className="font-semibold text-gray-800"><FaPills /> Current Medicines</h4>
                  ...
                  <div key={m.id} className="flex items-center justify-between border rounded-lg p-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="w-24 h-2 bg-gray-100 rounded-full mt-1 overflow-hidden">
                        <div className="h-full bg-teal-600" style={{ width: `${m.adherence}%` }} />
                      </div>
                      <div className="text-xs text-gray-500 mt-1">{m.adherence}%</div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-3">
                <button onClick={() => navigate("/prescriptions")} className="w-full bg-white border py-2 rounded-lg">All Prescriptions</button>
              </div>
            </div>

            <div className="bg-white rounded-2xl shadow p-4">
              <h4 className="font-semibold text-gray-800"><FaUpload /> Upload Reports</h4>
              <p className="text-sm text-gray-500 mt-1">Share lab results or scans for clinician review</p>
              <div className="mt-3 flex gap-2">
                <input ref={fileRef} onChange={handleFiles} type="file" multiple className="hidden" id="file-u" />
                <label htmlFor="file-u" onClick={() => fileRef.current?.click()} className="flex-1 bg-white border py-2 rounded-lg text-center cursor-pointer">Choose files</label>
                <button onClick={() => fileRef.current?.click()} className="bg-teal-600 text-white px-4 py-2 rounded-lg">Upload</button>
              </div>
            ...
            <div className="bg-gradient-to-b from-red-500 to-red-600 text-white rounded-2xl p-4 shadow">
              <div className="flex items-start gap-3">
                <div className="text-2xl"><FaPhoneAlt /></div>
                <div>
                  <div className="font-semibold">Emergency</div>
                  <div className="text-sm opacity-90">Ambulance and emergency contacts</div>
                  <div className="mt-3 flex gap-2">
                    <a href="tel:+919876543210" className="bg-white text-red-600 px-3 py-2 rounded-lg">Call</a>
                    <button onClick={() => alert("Emergency alert sent (demo)")} className="bg-white/10 border border-white/20 px-3 py-2 rounded-lg">Send Alert</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Additional insights: Symptom severity bar chart + health tips */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white rounded-2xl shadow p-4 col-span-2">
            <div className="flex items-center justify-between">
              <h4 className="font-semibold text-gray-800"><FaHeartbeat /> Symptom Severity (last 6 visits)</h4>
          ...
          <aside className="bg-white rounded-2xl shadow p-4">
            <h4 className="font-semibold text-gray-800">Health Tips</h4>
            <ul className="mt-3 space-y-2 text-sm text-gray-600">
              <li>Drink plenty of water and maintain salt balance.</li>
              <li>Cover standing water and use mosquito repellent in high-risk areas.</li>
              <li>Keep a digital copy of prescriptions and reports.</li>
              <li>Get seasonal flu vaccination if eligible.</li>
            </ul>
            <div className="mt-4">
              <button onClick={() => navigate("/learn")} className="w-full bg-teal-600 text-white py-2 rounded-lg">Learn more</button>
            </div>
          </aside>
        </div>

        <div className="text-center text-sm text-gray-500 pb-8">
          © {new Date().getFullYear()} SehatSathi — Demo content
        </div>
      </main>

      {/* Floating quick actions (mobile-friendly) */}
      <div className="fixed right-6 bottom-6 z-50 space-y-3">
        <button title="AI Symptom Checker" onClick={() => navigate("/symptom-checker")} className="bg-teal-600 text-white p-3 rounded-full shadow-lg hover:bg-teal-700">
          <FaRobot />
        </button>
        <button title="Book Appointment" onClick={() => navigate("/appointments")} className="bg-white p-3 rounded-full shadow-lg border">
          <FaPlus />
        </button>
        <button title="Upload Report" onClick={() => fileRef.current?.click()} className="bg-white p-3 rounded-full shadow-lg border">
          <FaUpload />
        </button>
        <button title="SOS Call" onClick={() => window.location.href = "tel:+919876543210"} className="bg-red-500 text-white p-3 rounded-full shadow-lg">
          <FaPhoneAlt />
        </button>
      </div>
    </div>
